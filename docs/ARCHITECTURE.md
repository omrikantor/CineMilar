...
- **Groq API** - handles the genuinely subjective part (interpreting *why* someone liked something) that a fixed algorithm can't do well, applied only to a TMDB-verified candidate set. Chosen over Gemini after hitting Gemini's free-tier daily quota (20 requests/day) during development - Groq's free tier allows 1,000 requests/day on this model, with no billing setup required. Getting it fully reliable on the free tier took real tuning, worth documenting: `openai/gpt-oss-20b` is a *reasoning* model that generates an internal chain-of-thought before its actual answer, and both count against `max_tokens` - when reasoning ran long it silently truncated our JSON output. Diagnosed by logging raw API responses rather than guessing, then fixed with `reasoning_effort: "low"` and `include_reasoning: false`, alongside a smaller candidate pool and truncated overview text to stay within the free tier's tokens-per-minute limit.
